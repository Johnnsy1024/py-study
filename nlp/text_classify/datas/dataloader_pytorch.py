...
class TrainDataSet(Dataset):
    def __init__(self, train_idx):
        ...
        self.feature = np.load(os.path.join(current_path, '1_char_src/feature.npy'))[train_idx]
        self.label = np.load(os.path.join(current_path, '1_char_src/label.npy'))[train_idx]
        with open(os.path.join(current_path, '1_char_src/THUCNews/vocab.pkl'), 'rb') as f:
...
def split_train_test(train_size_ratio: float):
    current_path = os.path.dirname(__file__)
    raw_data = np.load(os.path.join(current_path, '1_char_src/feature.npy'))
