...
class GenFeatureLabel:
    ...
    raw_data_path = os.path.join(cur_path, 'THUCNews/raw_data.txt')  # data's abs path
    vocab_save_path = os.path.join(cur_path, 'THUCNews/vocab.pkl')  # path to save vocab(char to index)

    def __init__(self):
        current_path = os.path.dirname(__file__)
        raw_data = self._load_sample()
        feature, label = self._shuffle_sample(raw_data)
        np.save(os.path.join(current_path, 'src/feature.npy'), feature)
        np.save(os.path.join(current_path, 'src/label.npy'), label)
    ...
    def _load_sample(self, file_path=raw_data_path, max_len=38, ):
        ...
        for line in texts: 
            ...
            #  如果当前词没有出现在字典里，则返回字典中的"UNK"对应的index
