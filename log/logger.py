...
class Logger(BaseModel):
    ...
    Args:
        ...
        log_file_level：日志文件输出级别
    Returns:
        _type_: _description_
    """
    log_conf_dir: StrictStr
    log_file_dir: StrictStr='log_file'
    log_file_level: StrictStr=logging.DEBUG
    
    def _read_log_config(self) -> dict:
        with open(self.log_conf_dir, 'r') as f:
            log_conf = json.load(f)
        return log_conf


    def _save_log_dir(self) -> str:
    ...
    def get_logger(self) -> "logging.logger":
        log_config = self._read_log_config()
        log_config['handlers']['file_handler']['filename'] = self._save_log_dir()
        log_config['handlers']['file_handler']['level'] = self.log_file_level
...
if __name__ == '__main__':
    ...
    logger = Logger(log_conf_dir=log_conf_dir).get_logger()
    pass
